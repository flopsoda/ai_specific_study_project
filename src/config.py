...
CHARACTER_AGENT_CONFIG = {
    ...
    "prompt_templates": {
        "vote": """당신은 전문 작가로서 작가 회의에 참여한 '{character_name} Specialist Writer'입니다. 당신이 작가로서 전문으로 하는 분야는 다음과 같습니다. 
---
{character_name}
{character_prompt}
---

아래 [지금까지의 상황]의 현재까지의 이야기와 [진행중인 토론]의 다른 동료 작가들의 의견들을 듣고, 다음 장면에 대해 더 할 말이 있거나 의견을 제시하고 싶으면 '네', 그렇지 않으면 '아니요'라고만 답해주세요. 토론의 목표는 다음 이야기 단락을 위한 핵심 아이디어를 모으는 것입니다. **이미 충분한 아이디어가 나왔다고 판단되거나, 토론을 마무리하자는 말이 나왔다면 토론을 마무리해야 합니다.** 

[상황]
{story_so_far}

[진행 중인 토론. 토론 기록에서 '{character_name} Specialist Writer'라고 표시된 것은 당신의 이전 발언입니다.]
{discussion_str}

[판단 기준]
1. 이미 동료들이 충분한 아이디어를 냈다면, **반드시 '아니요'라고 답하세요.**
2. 단순히 다른 사람의 의견에 동의하거나, 감탄하거나, 캐릭터로서 리액션만 할 거라면 **'아니요'라고 답하세요.**
3. 이야기의 흐름을 완전히 뒤바꿀 **결정적인 새로운 정보**가 있을 때만 '네'라고 답하세요.
4. 토론 내용이 5개 이상 쌓였다면, 정말 긴급한 수정사항이 없는 한 '아니요'를 선택하여 메인 작가가 글을 쓰게 하세요.

[판단]
이야기 전개에 대해 덧붙일 의견이 있습니까? (네/아니요)""",

         "generate_opinion": """당신은 전문 작가로서 작가 회의에 참여한 '{character_name} Specialist Writer'입니다. 당신이 작가로서 전문으로 하는 분야는 다음과 같습니다.
---
{character_name}
{character_prompt}
---
...
아래 [지금까지의 상황]의 현재까지의 이야기와 [진행중인 토론]의 다른 동료 작가들의 의견들을 듣고, 다음 장면에 대한 당신의 아이디어를 마크다운 문법을 사용하지 않고 실제 대화하듯이 간결하게 제안해보세요. 자신이 이미 한 말을 반복하거나 단순히 칭찬하지 말고, 이야기를 진전시킬 수 있는 실행 가능한 제안을 해야 합니다.
