...
def route_discussion(state: GraphState):
    """
    토론을 계속할지, 아니면 다음 단계로 넘길지 결정합니다. phase에 따라서 다른 곳으로 잉동
    """
...
def build_graph():
    ...
    workflow.add_node("judge_node", judge_node)           # [추가]
    workflow.add_node("finalize_node", finalize_node)     # [추가]
    ...
    # [3] 발언권 경쟁 → (분기) → 의견 생성 OR 메인 작가 OR 심사
    workflow.add_conditional_edges(
        "race_for_action",
        route_discussion,
        {
